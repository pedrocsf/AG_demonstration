...
const FEATURE_PT = [
  "Gestações", "Glicose", "Pressão arterial", "Espessura da pele",
  "Insulina", "IMC", "Histórico familiar", "Idade",
];
const N_FEATURES = 8;
...
async function carregarDados() {
  ...
  if (!resp.ok) throw new Error("Falha ao baixar o dataset (HTTP " + resp.status + ")");
...
function prepararDados(rawX, rawY, r) {
  ...
  for (const row of Xtr) for (const c of COLS_TO_IMPUTE) if (row[c] === 0) row[c] = medians[c];
  for (const row of Xte) for (const c of COLS_TO_IMPUTE) if (row[c] === 0) row[c] = medians[c];
  ...
  return {
    XtrScaled, XteScaled, yTrain, yTest, cvFolds, tBounds, stdsTrain,
    means, stds,
  };
}
...
function copiar(ind) {
  return {
    s: ind.s.slice(), d: ind.d.slice(), t: ind.t.slice(),
    w: ind.w.slice(), k: ind.k, fitness: ind.fitness,
  };
}

function crossover(p1, p2, crossoverRate) {
  if (rng() < crossoverRate) {
    const c1 = copiar(p1), c2 = copiar(p2);
    ...
    c1.s = nb1.slice(0, 8); c1.d = nb1.slice(8, 16);
    c2.s = nb2.slice(0, 8); c2.d = nb2.slice(8, 16);

    // BLX-alpha para valores reais (t e w)
    const alpha = 0.5;
    for (let i = 0; i < N_FEATURES; i++) {
      let cmin = Math.min(p1.t[i], p2.t[i]), cmax = Math.max(p1.t[i], p2.t[i]);
      let diff = cmax - cmin;
      c1.t[i] = cmin - alpha * diff + rng() * (diff * (1 + 2 * alpha));
      c2.t[i] = cmin - alpha * diff + rng() * (diff * (1 + 2 * alpha));
      let wmin = Math.min(p1.w[i], p2.w[i]), wmax = Math.max(p1.w[i], p2.w[i]);
      let wdiff = wmax - wmin;
      c1.w[i] = clip(wmin - alpha * wdiff + rng() * (wdiff * (1 + 2 * alpha)), 0, 1);
      c2.w[i] = clip(wmin - alpha * wdiff + rng() * (wdiff * (1 + 2 * alpha)), 0, 1);
    }
    // BLX-alpha para o limiar de inferência k
    let kmin = Math.min(p1.k, p2.k), kmax = Math.max(p1.k, p2.k);
    let kdiff = kmax - kmin;
...
function iniciarEvolucao(cfg) {
  ...
  const pop = Array.from({ length: cfg.popSize }, () => novoIndividuo(PREP.tBounds));
...
function passoGeracao() {
  ...
  for (const ind of pop) {
    if (ind.fitness === -1.0)
      ind.fitness = calcularAptidao(ind, PREP.XtrScaled, PREP.yTrain, PREP.cvFolds);
  }
  ...
  // Critérios de parada
  const parar =
    state.semMelhora >= cfg.patience || state.gen >= cfg.maxGen - 1;
  if (parar) {
    state.running = false;
    return;
  }
...
function desenharTudo() {
  ...
  setStatus(
    ...
    (state.running ? `estagnação ${state.semMelhora}/${state.cfg.patience}` : "parado")
  );
  desenharFitness();
  atualizarRegra(best);
...
function desenharFitness() {
  ...
  const padL = 38, padB = 22, padT = 12, padR = 10;
  ...
  const drawLine = (arr, color, width) => {
    ...
    ctx.lineWidth = width;
    ctx.beginPath();
    arr.forEach((v, i) => (i === 0 ? ctx.moveTo(px(i), py(v)) : ctx.lineTo(px(i), py(v))));
    ctx.stroke();
  };
...
function atualizarRegra(best) {
  ...
  for (let i = 0; i < N_FEATURES; i++) {
    ...
    row.querySelector(".rule-op").textContent = ativo ? (best.d[i] === 1 ? ">" : "<") : "—";
    row.querySelector(".rule-thr").textContent = ativo ? tOrig.toFixed(1) : "";
    row.querySelector(".rule-wfill").style.width = (best.w[i] * 100).toFixed(0) + "%";
    row.querySelector(".rule-wval").textContent = ativo ? best.w[i].toFixed(2) : "";
  }
...
function atualizarMetricas(teste) {
  const y = PREP.yTest, p = teste.preds;
...
function bindUI() {
  ...
  document.getElementById("clsStartBtn").addEventListener("click", async () => {
    ...
    const cfg = {
      popSize: clip(parseInt(document.getElementById("clsPop").value) || 80, 10, 500),
      maxGen: clip(parseInt(document.getElementById("clsGen").value) || 150, 1, 1000),
      patience: clip(parseInt(document.getElementById("clsPatience").value) || 25, 1, 200),
      crossoverRate: (parseFloat(document.getElementById("clsCrossover").value) || 80) / 100,
      mutationRate: (parseFloat(document.getElementById("clsMutation").value) || 2) / 100,
      genPerSec: clip(parseFloat(document.getElementById("clsSpeed").value) || 12, 1, 60),
    };
    iniciarEvolucao(cfg);
  });
